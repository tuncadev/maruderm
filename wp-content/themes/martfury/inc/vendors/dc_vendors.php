...
class Martfury_DCVendors {
	...
	function __construct() {
		...
		if ( class_exists( 'TAWC_Deals' ) ) {
			add_action( 'mvx_afm_product_options_pricing', array( $this, 'product_manage_fields_pricing' ) );
		}

		add_action( 'mvx_afm_product_options_related', array( $this, 'product_manage_fields_fbt' ) );

		add_action( 'mvx_process_product_object', array( $this, 'process_product_save_object' ), 20, 2 );

		add_filter( 'mvx_frontend_dash_upload_script_params', array( $this, 'frontend_dash_upload_script_params' ) );
	...
	function product_loop_sold_by() {
		...
		$sold_by_text = apply_filters( 'mvx_sold_by_text', esc_html__( 'Sold By:', 'martfury' ) );
