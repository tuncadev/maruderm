...
function martfury_get_option( $name ) {
	...
	if ( class_exists( 'Kirki' ) ) {
		...
		$default = martfury_get_option_default( $name );
...
function martfury_get_option_default( $name ) {
	...
	if ( class_exists( 'Kirki' ) && isset( \Kirki::$fields[ $name ] ) && isset( \Kirki::$fields[ $name ]['default'] ) ) {
		...
		$settings = martfury_customize_settings();
		$default = isset( $settings['fields'][ $name ]['default'] ) ? $settings['fields'][ $name ]['default'] : false;
	}
...
function martfury_customize_settings() {
	...
	if ( function_exists('wcboost_products_compare') ) {
		$sections['compare_page'] = array(
			'title'    => esc_html__( 'Compare Page', 'martfury' ),
			'panel'    => 'woocommerce',
		);
	}
	...
	$fields = array(
		...
		'search_columns'                   => array(
			'type'        => 'multicheck',
			'label'       => esc_html__( 'Search Columns', 'martfury' ),
			'section'     => 'header_search_content',
			'default'     => array( 'post_title', 'post_excerpt', 'post_content' ),
			'priority'    => 90,
			'choices'     => array(
				'post_title'  => esc_html__( 'Title', 'martfury' ),
				'post_excerpt' => esc_html__( 'Excerpt', 'martfury' ),
				'post_content'   => esc_html__( 'Content', 'martfury' ),
			),
			'description' => esc_html__( 'Select which columns you want to search.', 'martfury' ),
		),
		...
		'wishlist_notice_auto_hide_custom'            => array(
			'type'     => 'custom',
			'section'  => 'product_notifications',
			'default'  => '<hr>',
			'priority' => 70,
		...
		'added_to_compare_notice'                => array(
			'type'        => 'toggle',
			'label'       => esc_html__( 'Added to Compare Notification', 'martfury' ),
			'description' => esc_html__( 'Display a notification when a product is added to compare', 'martfury' ),
			'section'     => 'product_notifications',
			'priority'    => 70,
			'default'     => 0,
		),
		'compare_notice_auto_hide'               => array(
			'type'        => 'number',
			'label'       => esc_html__( 'Compare Notification Auto Hide', 'martfury' ),
			'description' => esc_html__( 'How many seconds you want to hide the notification.', 'martfury' ),
			'section'     => 'product_notifications',
			'priority'    => 70,
			'default'     => 3,
		...
		'taxonomy_description_first_page_10'                   => array(
			'type'        => 'toggle',
			'label'       => esc_html__( 'Taxonomy Description First page', 'martfury' ),
			'description' => esc_html__( 'Check this option to show the taxonomy description for the first page only.', 'martfury' ),
			'section'     => 'catalog_general',
			'default'     => true,
			'priority'    => 70,
		),
		...
		'product_360_view'       => array(
			'type'        => 'toggle',
			'label'       => esc_html__( 'Product 360 view', 'martfury' ),
			'section'     => 'product_page',
			'default'     => 1,
			'priority'    => 40,
		),
