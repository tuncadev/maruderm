...
final class LandingPageRenderer
{
    ...
    public function __construct(?LandingPageCatalog $catalog = null, ?LandingPageContent $content = null)
    {
    ...
    private function renderHero(array $categories): void
    {
        ...
        $category_url = isset($categories[0]) ? $this->catalog->categoryUrl($categories[0]) : $shop_url;

        echo '<section class="md-hero md-reveal">';
        echo '<div class="md-shell md-hero__grid">';
        echo '<div class="md-hero__copy">';
        echo '<span class="md-kicker">' . esc_html($hero['eyebrow']) . '</span>';
        echo '<h1>' . wp_kses($hero['title'], ['em' => []]) . '</h1>';
        ...
        echo '</div>';
        echo '<div class="md-hero__proof"><span><strong>149+</strong> засобів</span><span><strong>5</strong> категорій догляду</span></div>';
        echo '</div>';
    ...
    private function renderCategoryRail(array $categories): void
    {
        ...
        echo $this->sectionHeading('Обирай за категорією', 'З чого почнемо?', 'md-categories-title', wc_get_page_permalink('shop'));
        ...
        foreach ($categories as $index => $category) {
            ...
            echo '<span class="md-category-card__image"><img src="' . esc_url($this->catalog->categoryImage($category)) . '" alt="" loading="lazy"></span>';
            echo '<span class="md-category-card__meta"><strong>' . esc_html($category->name) . '</strong><small>' . esc_html((string) $category->count) . ' засобів</small></span>';
            echo $this->arrowIcon();
            echo '</a>';
        }
    ...
    private function renderProductShelf(string $eyebrow, string $title, array $products, string $tone): void
    {
        if ($products === []) {
            return;
        }
        ...
        echo '<div class="md-shell">';
        echo $this->sectionHeading($eyebrow, $title, $section_id . '-title', wc_get_page_permalink('shop'));
    ...
    private function renderProductCard(\WC_Product $product): void
    {
        ...
        echo '<article class="md-product-card">';
    ...
    private function renderEditorial(array $categories): void
    {
        ...
        echo '<section class="md-section md-editorial md-reveal" aria-label="Тематичні добірки"><div class="md-shell md-editorial__grid">';
        echo '<a class="md-story md-story--large" href="' . esc_url($this->catalog->categoryUrl($primary)) . '">';
        echo '<img src="' . esc_url($this->catalog->categoryImage($primary, 'large')) . '" alt="" loading="lazy">';
        ...
        echo '<img src="' . esc_url($this->catalog->categoryImage($secondary, 'large')) . '" alt="" loading="lazy"><span class="md-story__scrim"></span><span class="md-story__copy"><small>Колір і догляд</small><strong>' . esc_html($secondary->name) . '</strong><span>Дивитися ' . $this->arrowIcon() . '</span></span>';
        echo '</a></div></div></section>';
    }
    ...
    private function renderRoutine(): void
    {
        ...
        echo '<div class="md-routine__intro"><span class="md-kicker">Три прості кроки</span><h2 id="md-routine-title">Рутина без перевантаження</h2><p>Послідовність, яку легко підтримувати щодня.</p></div>';
    ...
    private function renderClosingCallout(): void
    {
        ...
        echo '<div><span class="md-kicker">Твій догляд, твої правила</span><h2>Знайди формули, які хочеться використовувати щодня.</h2></div>';
        echo '<a class="md-button md-button--light" href="' . esc_url(wc_get_page_permalink('shop')) . '">Перейти до каталогу' . $this->arrowIcon() . '</a>';
        echo '</div></div></section>';
    }

    private function sectionHeading(string $eyebrow, string $title, string $id, string $url): string
    {
        return '<header class="md-section-heading"><div><span class="md-kicker">' . esc_html($eyebrow) . '</span><h2 id="' . esc_attr($id) . '">' . esc_html($title) . '</h2></div><a class="md-text-link" href="' . esc_url($url) . '">Переглянути все' . $this->arrowIcon() . '</a></header>';
    }
