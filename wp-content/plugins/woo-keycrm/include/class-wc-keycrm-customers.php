 ...
 * KeyCRM Integration.
 *
 * @package  WC_Keycrm_Customers
 * @category Integration
 * @author   KeyCRM
 */

if (!class_exists('WC_Keycrm_Customers')) :

    /**
     * Class WC_Keycrm_Customers
     */
    class WC_Keycrm_Customers
    {
        /**
        ...
        /** @var bool | WC_Keycrm_Proxy | \WC_Keycrm_Client_V5 */
        protected $keycrm;

        /** @var array */
        protected $keycrm_settings = array();

        /** @var WC_Keycrm_Customer_Address */
        ...
        /**
         * WC_Keycrm_Customers constructor.
         *
         * @param bool | WC_Keycrm_Proxy     $keycrm
         * @param array                         $keycrm_settings
         * @param WC_Keycrm_Customer_Address $customer_address
         */
        public function __construct($keycrm, $keycrm_settings, $customer_address)
        {
            $this->keycrm = $keycrm;
        ...
        {
            if ($address instanceof WC_Keycrm_Customer_Address) {
        ...
        public function isCorporateEnabled()
        {
            ...
            return $this->keycrm->getCorporateEnabled();
        }
        ...
        public function customersUpload($ids = array())
        {
            ...
            foreach ($data as $array_customers) {
                $this->keycrm->customersUpload($array_customers);
        ...
        public function createCustomer($customer, $order = null)
        {
            ...
            if ($this->isCustomer($customer)) {
                ...
                $response = $this->keycrm->customersCreate($this->customer);
        ...
        public function updateCustomer($customer_id)
        {
            ...
            if ($this->isCustomer($customer)) {
                $this->processCustomer($customer);
                $this->keycrm->customersEdit($this->customer);
            }
        ...
        public function updateCustomerById($customer_id, $crmCustomerId)
        {
            ...
            if ($this->isCustomer($customer)) {
                ...
                $this->keycrm->customersEdit($this->customer, 'id');
            }
        ...
        public function createCorporateCustomerForOrder($crmCustomerId, $customer, $order)
        {
            ...
            if ($this->isCustomer($customer)) {
                ...
                $response = $this->keycrm->customersCorporateCreate($this->customerCorporate);
        ...
        {
            $found = false;
            $builder = new WC_Keycrm_Customer_Corporate_Address();
            $newAddress = $builder
                ->setFallbackToShipping(true)
                ->setIsMain(false)
                ->setExplicitIsMain(false)
                ->setWCAddressType(WC_Keycrm_Abstracts_Address::ADDRESS_TYPE_BILLING)
                ->build($customer, $order)
                ->get_data();
            $addresses = $this->keycrm->customersCorporateAddresses(
                $corporateId,
                array(),
                null,
                100,
            ...
            if (!$found) {
                $this->keycrm->customersCorporateAddressesCreate(
                    $corporateId,
                    $newAddress,
                    'id',
                    $this->keycrm->getSingleSiteForKey()
                );
            }
        }
         ...
         *
         * @param \WC_Keycrm_Response $response
         *
        ...
        protected function fillCorporateCustomer($response)
        {
            ...
            $response = $this->keycrm->customersCorporateAddressesCreate(
                $customerId,
            ...
            if ($response->isSuccessful() && $response->offsetExists('id')) {
                ...
                $this->keycrm->customersCorporateCompaniesCreate(
                    $customerId,
        ...
        protected function processCustomer($customer, $order = null)
        {
            ...
            $this->customer = apply_filters(
                'keycrm_process_customer',
                WC_Keycrm_Plugin::clearArray($data_customer),
                $customer
            );
        }
        ...
        protected function processCorporateCustomer($crmCustomerId, $customer, $order)
        {
            ...
            $orderAddress = new WC_Keycrm_Order_Address();
            $corpAddress = new WC_Keycrm_Customer_Corporate_Address();
            ...
            $this->customerCorporate = apply_filters(
                'keycrm_process_customer_corporate',
                WC_Keycrm_Plugin::clearArray($data_customer),
                $customer
            );
            $this->customerCorporateAddress = apply_filters(
                'keycrm_process_customer_corporate_address',
                WC_Keycrm_Plugin::clearArray(array_merge(
                    $shippingAddress,
                    array('isMain' => true)
                )),
                $customer
            ...
            $this->customerCorporateCompany = apply_filters(
                'keycrm_process_customer_corporate_company',
                WC_Keycrm_Plugin::clearArray($data_company),
                $customer
            );
        }
        ...
        private function searchCustomer($filter)
        {
            if (isset($filter['externalId'])) {
                $search = $this->keycrm->customersGet($filter['externalId']);
            } elseif (isset($filter['email'])) {
                if (empty($filter['email']) && count($filter) == 1) {
                    return false;
                }

                $search = $this->keycrm->customersList(array('email' => $filter['email']));
            }
        ...
        {
            $search = $this->keycrm->customersCorporateList($filter);
        ...
        {
            $keycrmSettings = array();

            if (!empty($this->keycrm_settings) && array_key_exists('client_roles', $this->keycrm_settings)) {
                $keycrmSettings = $this->keycrm_settings['client_roles'];
            }

            if (empty($keycrmSettings)) {
                $selectedRoles = array(self::CUSTOMER_ROLE, self::ADMIN_ROLE);
            } else {
                $selectedRoles = $keycrmSettings;
            }
