 ...
 * KeyCRM Integration.
 *
 * @package  WC_Keycrm_History
 * @category Integration
 * @author   KeyCRM
 */

if ( ! class_exists( 'WC_Keycrm_History' ) ) :

    /**
     * Class WC_Keycrm_History
     */
    class WC_Keycrm_History
    {
        /** @var \DateTime */
        ...
        {
            $this->keycrmSettings = get_option(WC_Keycrm_Base::$option_key);

            if (isset($this->keycrmSettings['bind_by_sku'])
                && $this->keycrmSettings['bind_by_sku'] == WC_Keycrm_Base::YES
            ) {
                $this->bindField = 'xmlId';
            }

            if (isset($this->keycrmSettings['order_methods'])) {
                $this->orderMethods = $this->keycrmSettings['order_methods'];
                unset($this->keycrmSettings['order_methods']);
            }
        ...
        {
            $ordersSinceId = get_option('keycrm_orders_history_since_id');
            $customersSinceId = get_option('keycrm_customers_history_since_id');

            if (!$ordersSinceId && isset($this->keycrmSettings['history_orders'])) {
                $this->startDateOrders = new DateTime($this->keycrmSettings['history_orders']);
            }

            if (!$customersSinceId && isset($this->keycrmSettings['history_customers'])) {
                $this->startDateCustomers = new DateTime($this->keycrmSettings['history_customers']);
            }
        ...
        protected function customersHistory($date, $sinceId)
        {
            ...
            $request = new WC_Keycrm_Paginated_Request();
            $history = $request
                ->setApi($this->keycrm)
            ...
            if (!empty($history)) {
                ...
                $customers = WC_Keycrm_History_Assembler::assemblyCustomer($history);
                WC_Keycrm_Plugin::$history_run = true;
                WC_Keycrm_Logger::debug(__METHOD__, array('Assembled customers history:', $customers));
                ...
                foreach ($customers as $crmCustomer) {
                    ...
                    try {
                        ...
                        if (!$builder->loadExternalId($crmCustomer['externalId'])) {
                            WC_Keycrm_Logger::addCaller(__METHOD__, sprintf(
                        ...
                        WC_Keycrm_Logger::debug(__METHOD__, array('Updated WC_Customer:', $wcCustomer));
                    } catch (\Exception $exception) {
                        WC_Keycrm_Logger::error(sprintf(
                        ...
                        WC_Keycrm_Logger::error($exception->getTraceAsString());
                    }
                }

                update_option('keycrm_customers_history_since_id', $lastChange['id']);
                WC_Keycrm_Plugin::$history_run = false;
            }
        }
        ...
        protected function ordersHistory($date, $sinceId)
        {
            ...
            unset($this->keycrmSettings['client_roles']);

            $options = array_flip(array_filter($this->keycrmSettings));
            ...
            if (!empty($history)) {
                $lastChange = end($history);
                $historyAssembly = WC_Keycrm_History_Assembler::assemblyOrder($history);
                WC_Keycrm_Logger::debug(__METHOD__, array('Assembled orders history:', $historyAssembly));
                WC_Keycrm_Plugin::$history_run = true;

                foreach ($historyAssembly as $orderHistory) {
                    $order = WC_Keycrm_Plugin::clearArray(apply_filters('keycrm_history_before_save', $orderHistory));
                ...
                update_option('keycrm_orders_history_since_id', $lastChange['id']);
                WC_Keycrm_Plugin::$history_run = false;
            }
        ...
        protected function updateShippingItemId($order, $options, $wcOrder)
        {
            ...
            $activeNetCost = $this->keycrmSettings['send_delivery_net_cost'];
            ...
            if (isset($order['delivery']['service']['code'])) {
                $service = keycrm_get_delivery_service($shipping->get_method_id(), $order['delivery']['service']['code']);
            ...
            if (array_key_exists('items', $order)) {
                foreach ($order['items'] as $key => $item) {
                    ...
                    if (isset($item['create']) && $item['create'] == true) {
                        ...
                        foreach ($wcOrder->get_items() as $orderItemId => $orderItem) {
                            if (
                                isset($this->keycrmSettings['bind_by_sku'])
                                && $this->keycrmSettings['bind_by_sku'] == WC_Keycrm_Base::YES
                            ) {
        ...
        private function deleteOrUpdateOrderItem($item, $orderItem, $orderItemId)
        {
            if (isset($item['delete']) && $item['delete'] == true) {
                ...
                if (isset($item['quantity']) && $item['quantity']) {
                    ...
                    $product = keycrm_get_wc_product($item['offer'][$this->bindField], $this->keycrmSettings);
        ...
        protected function orderCreate($order, $options)
        {
            ...
            $orderResponse = $this->keycrm->ordersGet($order['id'], 'id');
            ...
            if ($this->keycrm->getCorporateEnabled()) {
            ...
            if ($this->keycrm->getCorporateEnabled() && self::isOrderCorporate($order)) {
                if (isset($order['contact'])) {
                    $contactOrCustomer = $order['contact'];

                    if (self::noRealDataInEntity($contactOrCustomer)) {
                        $response = $this->keycrm->customersGet($contactOrCustomer['id'], 'id');
            ...
            if ($productData) {
                foreach ($productData as $key => $product) {
                    ...
                    $arItemsNew = array();
                    $arItemsOld = array();
                    $item = keycrm_get_wc_product($product['offer'][$this->bindField], $this->keycrmSettings);

                    if (!$item) {
                        $logger = new WC_Logger();
                        $logger->add('keycrm', 'Product not found by ' . $this->bindField);
                        continue;
                    }
            ...
            if (array_key_exists('delivery', $order)) {
                ...
                if ($deliveryCode && isset($options[$deliveryCode])) {
                    ...
                    if (isset($order['delivery']['service']['code'])) {
                        $service = keycrm_get_delivery_service(
            ...
            $this->keycrm->ordersFixExternalIds($ids);
        ...
        protected function editOrder($order, $event = 'create')
        {
            ...
            if ($event == 'update') {
                $result = $this->keycrm->ordersGet($order['externalId']);
            ...
            foreach ($iterableItems as $id => $item) {
                ...
                if (!isset($order['items'][$item['id']])) {
                    if (empty($crmOrder)) {
                        $result = $this->keycrm->ordersGet($order['id'], 'id');
            ...
            if (!empty($orderItems)) {
                $orderEdit = array(
                    'id' => $order['id'],
                    'items' => WC_Keycrm_Plugin::clearArray($orderItems),
                );

                $this->keycrm->ordersEdit($orderEdit, 'id');
            }
        }
        ...
        protected function handleCustomerDataChange($wcOrder, $order)
        {
            ...
            $crmOrder = array();
            $newCustomerId = null;
            $switcher = new WC_Keycrm_Customer_Switcher();
            $data = new WC_Keycrm_Customer_Switcher_State();
            ...
            if (isset($order['customer'])) {
                ...
                if (empty($crmOrder)) {
                    WC_Keycrm_Logger::addCaller(__METHOD__, sprintf(
                        'Cannot get order data from keyCRM. Skipping customer change. History data: %s',
                        print_r($order, true)
                    ));
            ...
            if ($data->feasible()) {
                try {
                    ...
                    WC_Keycrm_Logger::addCaller(__METHOD__, $errorMessage);
                    WC_Keycrm_Logger::debug(__METHOD__, sprintf(
                        '%s%s%s',
                        $errorMessage,
                        PHP_EOL,
        ...
        /**
         * Returns keyCRM order by id or by externalId.
        ...
        {
            $crmOrderResponse = $this->keycrm->ordersGet($id, $by);
         ...
         * @param \WC_Keycrm_Customer_Switcher_State $data
